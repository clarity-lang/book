...
describe("Testing vote", () => {
  test("Allows members to vote", () => {
    simnet.callPublicFn(
      "multisig-vault",
      "start",
      [members, Cl.uint(members.list.length)],
      deployer
    );
    simnet.callPublicFn(
      "multisig-vault",
      "deposit",
      [Cl.uint(stxVaultAmount)],
      deployer
    );
...
describe("Testing get-vote", () => {
  test("Can retrieve a member's vote for a principal", () => {
    ...
    const voteResponse = simnet.callReadOnlyFn(
      "multisig-vault",
      "get-vote",
      [Cl.principal(wallet1), Cl.principal(deployer)],
      deployer
    );
